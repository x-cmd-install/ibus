# ibus

[English version](./README.md)

Intelligent Input Bus for Linux/Unix

![ibus](https://repo.x-cmd.io/ibus.svg?lang=zh)

## 安装

```sh
x install ibus
```

## 代码洞察

合计: **123,558** 行代码（覆盖前 5 种语言、共 **324** 个文件）。

| 语言 | 代码 | 注释 | 空行 | 文件数 |
|------|-----:|-----:|-----:|------:|
| C | 56,979 | 7,006 | 8,021 | 106 |
| CHeader | 17,716 | 11,833 | 1,721 | 82 |
| Autoconf | 13,330 | 145 | 180 | 36 |
| Vala | 11,142 | 1,340 | 1,613 | 27 |
| Python | 10,320 | 1,784 | 1,767 | 73 |

## OpenSSF Scorecard 评分

总评分: **3.4 / 10**

评分最低的几项:

- **Token-Permissions** (0/10) — detected GitHub workflow tokens with excessive permissions
- **Packaging** (-1/10) — packaging workflow not detected
- **Code-Review** (0/10) — Found 1/24 approved changesets -- score normalized to 0

## 源代码

- **上游仓库**: <https://github.com/ibus/ibus>
- **官网**: <https://github.com/ibus/ibus/wiki>
- **许可证**: LGPL-2.1

## 发布

- **最新版本**: `1.5.35-beta2` (2026-04-03)
- **最近提交**: 2026-09-07
- **Release 含资产**: 4 个

## 流行度

- **Star**: 996 · **Fork**: 209 · **开放 issue**: 2,631 · **贡献者**: 4

## 累计统计

- **发布数**: 71 · **已合并 PR**: 59 · **开放 PR**: 15 · **已关闭 issue**: 2374 · **开放 issue**: 257 · **提交数**: 3606

## 最近活动

| 时间窗口 | 起始 | 发布 | 已合并 PR | 开放 PR | 已关闭 issue | 开放 issue | 提交 |
|---|---|---:|---:|---:|---:|---:|---:|
| 30d | 2026-08-12 | 2 | 3 | 4 | 4 | 5 | 15 |
| last60d | 2026-07-13 | 2 | 6 | 4 | 5 | 8 | 47 |
| 90d | 2026-06-13 | 3 | 7 | 4 | 8 | 11 | 56 |
| last180d | 2026-03-15 | 6 | 27 | 7 | 22 | 14 | 97 |
| 360d | 2025-09-16 | 8 | 34 | 9 | 50 | 23 | 186 |
| last720d | 2024-09-21 | 11 | 38 | 12 | 120 | 31 | 329 |

## Release 资产

| 资产 | 大小 | 目标平台 |
|------|-----:|----------|
| [ibus-1.5.34-rc2.tar.gz](https://github.com/ibus/ibus/releases/download/1.5.34/ibus-1.5.34-rc2.tar.gz) | 4.3 MiB | `native/unknown` |
| [ibus-1.5.34-rc2.tar.gz.sum](https://github.com/ibus/ibus/releases/download/1.5.34/ibus-1.5.34-rc2.tar.gz.sum) | 380 B | `other` |
| [ibus-1.5.34.tar.gz](https://github.com/ibus/ibus/releases/download/1.5.34/ibus-1.5.34.tar.gz) | 4.3 MiB | `native/unknown` |
| [ibus-1.5.34.tar.gz.sum](https://github.com/ibus/ibus/releases/download/1.5.34/ibus-1.5.34.tar.gz.sum) | 364 B | `other` |

## 发行版状态

在 [repology.org](https://repology.org/project/ibus) 上共有 **303** 个发行版报告此项目。**68** 个 ✅ 已是最新上游版本，**212** 个 ⚠️ 使用旧版本。

| 发行版 | 版本 | 状态 |
|--------|------|------|
| Debian unstable | `1.5.34` | ✅ latest |
| Debian 14 | `1.5.34` | ✅ latest |
| Debian 13 | `1.5.32` | ⚠️ outdated |
| Ubuntu 26.04 LTS | `1.5.34~rc2` | ⚠️ outdated |
| Ubuntu 24.04 LTS | `1.5.29` | ⚠️ outdated |
| Arch | `1.5.34` | ✅ latest |
| Fedora rawhide | `1.5.35~beta2` | devel |
| Nix unstable | `1.5.34` | ✅ latest |
| Void | `1.5.32` | ⚠️ outdated |
| Alpine edge | `1.5.34` | ✅ latest |
| openSUSE Tumbleweed | `1.5.34` | ✅ latest |

## 改进这些数据

ibus 的安装元数据由 [x-cmd/install](https://github.com/x-cmd/install) 索引维护——这是一份由 x-cmd 在安装时读取的精选 YAML 包列表。如果 `ibus` 缺失、过期，或安装行为有问题，欢迎在该 repo 提 issue 或 PR：

- **提交 issue**: <https://github.com/x-cmd/install/issues/new>
- **编辑包条目**: <https://github.com/x-cmd/install/edit/main/ibus.yml>（或索引实际使用的路径）

本页面的数据（card / loc / scorecard / release）由 [x-cmd-install-action](https://github.com/x-cmd-install/x-cmd-install-action) 自动采集，每日重新生成。**安装行为**（版本选择、平台差异、依赖处理）的改进应提交到上游索引。

_数据快照: `data/card/260911.yml` · 2026-09-11T05:16:22Z._
